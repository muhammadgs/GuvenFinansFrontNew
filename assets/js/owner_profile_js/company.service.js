...
class CompaniesService {
    constructor() {
        ...
        this.isAddingCompanyByCode = false; // ⬅️ BU ƏLAVƏ EDİN
    ...
    setupEventListeners() {
        ...
        // Yeni düymə: Şirkət kodu ilə əlavə et
        const addByCodeBtn = document.getElementById('addCompanyByCodeBtn');
        if (addByCodeBtn) {
            addByCodeBtn.addEventListener('click', () => this.openCompanyCodeModal());
        }

        // Əgər bu düymə yoxdursa, mövcud əlavə et düyməsinin yanına əlavə et
        if (!addByCodeBtn) {
            this.createAddByCodeButton();
        }
     ...
     * ƏLAVƏ DÜYMƏ YARAT
     */
    createAddByCodeButton() {
        // Mövcud əlavə et düyməsinin yanına yeni düymə əlavə et
        const existingAddBtn = document.getElementById('addCompanyBtn') ||
                              document.getElementById('modalAddCompanyBtn');

        if (existingAddBtn) {
            const newButton = document.createElement('button');
            newButton.id = 'addCompanyByCodeBtn';
            newButton.className = 'px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl hover:opacity-90 transition font-medium flex items-center';
            newButton.innerHTML = `
                <i class="fa-solid fa-link mr-2"></i>
                Şirkət Kodundan Əlavə Et
            `;

            // Düyməni əlavə et
            existingAddBtn.parentNode.insertBefore(newButton, existingAddBtn.nextSibling);

            // Event listener əlavə et
            newButton.addEventListener('click', () => this.openCompanyCodeModal());
        }
    }
    ...
    async handleAddCompanyByCode() {
        if (this.isAddingCompanyByCode) return;
        ...
        try {
            console.log('🔗 Mövcud şirkət alt şirkət edilir...');

            // 1. Cari user-in şirkət kodunu al
            const userData = JSON.parse(localStorage.getItem('userData') || '{}');
            const userCompanyCode = userData.company_code || userData.user?.company_code;

            if (!userCompanyCode) {
                throw new Error('Sizin şirkət kodunuz tapılmadı');
            }

            // 2. Şirkət kodunu götür
            ...
            // 3. API çağırışı
            const requestData = { child_company_code: childCompanyCode };
            console.log('📤 API-ə göndərilir:', requestData);

            const endpoint = `/companies/add_existing_as_subsidiary`;
            ...
            if (response?.success) {
                ...
                this.showSuccess(
                    `✅ ${response.child_company.name} artıq sizin alt şirkətinizdir!\n` +
                    `Kod: ${response.child_company.code}`
                );
            } else {
            ...
            this.showError(error.message || 'Şirkət əlavə edilmədi');
        } finally {
    ...
    createCompanyCodeModal() {
        const modalHTML = `
            <div id="companyCodeModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style="z-index: 9999;">                <div class="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl" style="z-index: 60;">
                <div class="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl" style="z-index: 10000;">
                    <!-- Header -->
                        ...
                        <form id="companyCodeForm">
                            <div class="mb-6">
                                ...
                                <input 
                                    ...
                                    placeholder="Məsələn: GAS26101"
                                    required
                                    autocomplete="off"
                                >
                                <p class="text-sm text-gray-500 mt-2">
                                    Alt şirkət etmək istədiyiniz şirkətin kodunu daxil edin
                                </p>
                            </div>
                            ...
                            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        <p class="text-sm text-blue-800 font-medium">Nə edəcək?</p>
                                        <p class="text-xs text-blue-700 mt-1">
                                            • Şirkət sizin alt şirkətiniz olacaq<br>
                                            • Şirkətin bütün məlumatları avtomatik gətiriləcək<br>
                                            • Yeni şirkət YARADILMAYACAQ<br>
                                            • Mövcud şirkət sadəcə sizin alt şirkətiniz olacaq
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-end space-x-3">
                                ...
                                <button 
                                    ...
                                    class="px-6 py-3 bg-gradient-to-r from-brand-blue to-blue-500 text-white rounded-xl hover:opacity-90 transition font-medium"
                                >
                                    <i class="fa-solid fa-link mr-2"></i>
                                    Alt Şirkət Et
                                </button>
    ...
    async handleAddCompany() {
        // ================== ÇAĞIRIŞ SAYINI İDARƏ ET ==================
        if (this.isAddingCompany) {
            console.log('⚠️ Şirkət əlavə edilir, təkrar çağırış qarşısı alındı');
            return;
        }
        ...
        try {
            console.log('➕ Yeni şirkət əlavə edilir...');

            // 1. Cari user-in şirkət kodunu al (SİZİN ŞİRKƏT KODUNUZ)
            let userCompanyCode = '';
            try {
                const userData = localStorage.getItem('userData');
                if (userData) {
                    ...
                    // Fərqli data strukturları üçün yoxlama
                    if (parsedData.user && parsedData.user.company_code) {
                        userCompanyCode = parsedData.user.company_code;
                    } else if (parsedData.company_code) {
                        userCompanyCode = parsedData.company_code;
                    } else if (parsedData.user && parsedData.user.companyCode) {
                        userCompanyCode = parsedData.user.companyCode;
                    } else if (parsedData.companyCode) {
                        userCompanyCode = parsedData.companyCode;
                    }

                    console.log('🏢 Cari user şirkət kodu (SİZİN KODUNUZ):', userCompanyCode);
                }
            } catch (userError) {
                console.error('❌ User data oxuma xətası:', userError);
                this.isAddingCompany = false;
                return;
            }

            if (!userCompanyCode) {
                this.showError('Sizin şirkət kodunuz tapılmadı. Yenidən giriş edin.');
                this.isAddingCompany = false;
                return;
            }

            // 2. Əvvəlcə bütün elementləri tap
            const elements = {
                name: document.getElementById('newCompanyName'),
                voen: document.getElementById('newCompanyVoen'),
                ceoEmail: document.getElementById('newCompanyCeoEmail'),
                phonePrefix: document.getElementById('newCompanyPhonePrefix'),
                ceoPhone: document.getElementById('newCompanyCeoPhone'), // 6 rəqəm
                ceoPassword: document.getElementById('newCompanyCeoPassword'),
                finCode: document.getElementById('newCompanyFinCode'),
                asanImza: document.getElementById('newCompanyAsanImza'),
                asanId: document.getElementById('newCompanyAsanId'),
                pin1: document.getElementById('newCompanyPin1'),
                pin2: document.getElementById('newCompanyPin2'),
                puk: document.getElementById('newCompanyPuk'),
                status: document.getElementById('newCompanyStatus'),
                address: document.getElementById('newCompanyAddress'),
                activityField: document.getElementById('newCompanyActivityField'),
                legalForm: document.getElementById('newCompanyLegalForm')
            };

            // 3. Elementlərin tapıldığını yoxla
            console.log('🔍 Elementlərin tapılması:');
            Object.keys(elements).forEach(key => {
                console.log(`${key}:`, elements[key] ? '✅' : '❌');
            });

            // 4. Əsas elementlərin tapılmadığını yoxla
            const requiredElements = ['name', 'voen', 'ceoEmail', 'phonePrefix', 'ceoPhone', 'ceoPassword', 'finCode'];
            const missingElements = requiredElements.filter(key => !elements[key]);

            if (missingElements.length > 0) {
                console.error('❌ Tapılmayan elementlər:', missingElements);
                this.showError('Form yüklənməyib. Səhifəni yeniləyin və yenidən cəhd edin.');
                this.isAddingCompany = false;
                return;
            }
            ...
            // Zəruri sahələr
            const name = getValue(elements.name);
            const voen = getValue(elements.voen);
            const ceoEmail = getValue(elements.ceoEmail);
            const phonePrefix = getValue(elements.phonePrefix, '050'); // Default 050
            const phoneNumberPart = getValue(elements.ceoPhone); // Nömrənin özü (6 rəqəm)
            const ceoPassword = getValue(elements.ceoPassword);
            const finCode = getValue(elements.finCode);
            const asanImza = getValue(elements.asanImza);
            const asanId = getValue(elements.asanId);
            const pin1 = getValue(elements.pin1);
            const pin2 = getValue(elements.pin2);
            const puk = getValue(elements.puk);

            // Opsional sahələr
            const status = getValue(elements.status, 'active');
            const address = getValue(elements.address);
            const activityField = getValue(elements.activityField);
            const legalForm = getValue(elements.legalForm);

            // 6. Telefon nömrəsini birləşdir (3 operator + 6 nömrə = 9 rəqəm)
            const ceoPhone = `+994${phonePrefix}${phoneNumberPart}`;

            console.log('📱 Telefon məlumatları:', {
                prefix: phonePrefix,
                numberPart: phoneNumberPart,
                full: ceoPhone,
                length: ceoPhone.length,
                expectedLength: 13 // +994 (4) + 9 rəqəm = 13
            });

            // 7. Validation - boş sahələri yoxla
            const requiredFields = [
                { value: name, name: 'Şirkət Adı' },
                { value: voen, name: 'VOEN' },
                { value: ceoEmail, name: 'CEO Email' },
                { value: phoneNumberPart, name: 'Telefon Nömrəsi' },
                { value: ceoPassword, name: 'CEO Şifrə' },
                { value: finCode, name: 'FİN Kod' },
                { value: asanImza, name: 'Asan İmza' },
                { value: asanId, name: 'Asan ID' },
                { value: pin1, name: 'PIN 1' },
                { value: pin2, name: 'PIN 2' },
                { value: puk, name: 'PUK' }
            ];

            const emptyFields = requiredFields.filter(item => !item.value || item.value.trim() === '');
            ...
            // 8. Format validation
            // VOEN validation - 10 rəqəm
            if (voen.length !== 10 || !/^\d+$/.test(voen)) {
                this.showError('VOEN tam 10 rəqəm olmalıdır');
                this.isAddingCompany = false;
                return;
            }
            ...
            if (!emailRegex.test(ceoEmail)) {
                this.showError('Düzgün email ünvanı daxil edin');
                this.isAddingCompany = false;
                return;
            }

            // Phone validation - nömrə hissəsi (6 rəqəm)
            if (phoneNumberPart.length !== 6 || !/^\d+$/.test(phoneNumberPart)) {
                this.showError('Telefon nömrəsi 6 rəqəm olmalıdır (format: 123456)');
                this.isAddingCompany = false;
                return;
            }

            // Tam telefon validation - 13 simvol (+994 + 9 rəqəm)
            const phoneRegex = /^\+994[0-9]{9}$/;
            if (!phoneRegex.test(ceoPhone)) {
                this.showError(
                    `Telefon nömrəsi formatı yanlışdır.\n` +
                    `Göndərilən: ${ceoPhone} (${ceoPhone.length} simvol)\n` +
                    `Tələb olunan: +994XXXXXXXXX (13 simvol)\n` +
                    `Nümunə: +994050123456`
                );
                this.isAddingCompany = false;
                return;
            ...
            // FIN Kod validation - max 7 simvol
            if (finCode.length > 7) {
                this.showError('FİN Kod maksimum 7 simvol olmalıdır');
                this.isAddingCompany = false;
                return;
            }

            // PIN validation - 4 rəqəm
            if (pin1.length !== 4 || !/^\d+$/.test(pin1)) {
                this.showError('PIN 1 tam 4 rəqəm olmalıdır');
                this.isAddingCompany = false;
                return;
            }

            if (pin2.length !== 4 || !/^\d+$/.test(pin2)) {
                this.showError('PIN 2 tam 4 rəqəm olmalıdır');
                this.isAddingCompany = false;
                return;
            }

            // PUK validation - 8 rəqəm
            if (puk.length !== 8 || !/^\d+$/.test(puk)) {
                this.showError('PUK tam 8 rəqəm olmalıdır');
                this.isAddingCompany = false;
                return;
            }

            console.log('✅ Validation keçdi:', {
                name, voen, ceoEmail, ceoPhone,
                finCode, asanImza, asanId, pin1, pin2, puk,
                address, activityField, legalForm, status,
                parentCompanyCode: userCompanyCode // ⚠️ SİZİN KODUNUZ
            });

            // 9. VOEN dublikat yoxlaması
            try {
                console.log(`🔍 VOEN yoxlanılır: ${voen}`);
                const voenCheck = await this.apiService.get(`/companies/check/voen/${voen}`);
                if (voenCheck && voenCheck.exists) {
                    this.showError('Bu VOEN artıq qeydiyyatdan keçib. Zəhmət olmasa fərqli VOEN daxil edin.');
                    this.isAddingCompany = false;
                    return;
                }
                console.log('✅ VOEN uyğundur');
            } catch (voenError) {
                console.log('⚠️ VOEN yoxlaması uğursuz oldu, davam edilir...', voenError);
            }

            // 10. Company məlumatlarını hazırla
            const newCompany = {
                company_name: name,
                voen: voen,
                ceo_email: ceoEmail,
                ...
                parent_company_code: userCompanyCode // ⚠️ BURAYA ƏLAVƏ EDİN
            };

            console.log('📤 API-ə göndərilir (ALT ŞİRKƏT):', {
                ...newCompany,
                ceo_password: '***', // Şifrəni gizlə
                parent_company_code: userCompanyCode
            });

            // 11. API çağırışı - ALT ŞİRKƏT ENDPOINT-İ
            if (!this.apiService) {
                this.showError('API servisi mövcud deyil');
                this.isAddingCompany = false;
                return;
            }

            try {
                // ⚠️ ALT ŞİRKƏT ENDPOINT-İ - parent_company_code artıq body-də göndərilir
                const endpoint = `/companies/create_company_subsidiaries`;
                console.log(`🌐 API endpoint: ${endpoint}`);
                console.log(`🏢 Alt şirkət yaradılır üst şirkət üçün: ${userCompanyCode}`);

                const response = await this.apiService.post(endpoint, newCompany);
                console.log('✅ API cavabı (alt şirkət):', response);
                ...
                if (response && response.company) {
                    ...
                    const countText = document.getElementById('companiesCountText');
                    if (countText) {
                        countText.textContent = `${this.companies.length} alt şirkət tapıldı`;
                    }

                    this.closeAddCompanyModal();
                    this.showSuccess(`✅ Yeni alt şirkət uğurla əlavə edildi!\n` +
                                   `Şirkət kodu: ${response.company.company_code}\n` +
                                   `Sizin alt şirkətiniz oldu.`);
                    ...
                    this.showError('Alt şirkət əlavə edilmədi: ' + (response?.message || 'Xəta baş verdi'));
                }
            } catch (apiError) {
                console.error('❌ API xətası:', apiError);

                // API error məlumatını göstər
                if (apiError.response && apiError.response.data) {
                    const errorData = apiError.response.data;
                    if (errorData.detail) {
                        if (Array.isArray(errorData.detail)) {
                            const errors = errorData.detail.map(e => e.msg).join('\n');
                            this.showError('Validation xətası:\n' + errors);
                        } else {
                            this.showError('Xəta: ' + errorData.detail);
                        }
                    } else {
                        this.showError('API xətası: ' + apiError.message);
                    }
                } else {
                    this.showError('API xətası: ' + (apiError.message || 'Xəta baş verdi'));
                }
            }
